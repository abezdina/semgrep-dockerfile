...
let map_anon_choice_param_88d8a28 (env : env) (x : CST.anon_choice_param_88d8a28) =
  (match x with
  | `Param x -> R.Case ("Param",
      map_param env x
    )
  | `Flag_param (v1, v2) -> R.Case ("Flag_param",
      let v1 = (* "--" *) token env v1 in
      let v2 = map_imm_tok_pat_f43f746 env v2 in
      R.Tuple [v1; v2]
    )
...
let map_copy_instruction (env : env) ((v1, v2, v3, v4, v5) : CST.copy_instruction) =
  ...
  let v2 =
    R.List (List.map (map_anon_choice_param_88d8a28 env) v2)
  in
  let v3 =
    R.List (List.map (fun (v1, v2) ->
